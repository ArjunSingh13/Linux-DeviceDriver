...
#include <linux/platform_device.h> // add this file for platform support
 ...
 * For device create dynamically when probe is called 
 **/
 ...
 *For driver create one variable globally
 * */
 ...
 *
 * Every device holds 2 memory structures device private structure and buffer, both
 * are allocated in probe function. now here we free the memory.
 * */
...
int pcd_platform_driver_probe(struct platform_device *pdev)
{
	...
	/* device private data structure is the one we prepare here as its unique for each device using platform driver */
	struct pcdev_private_data *dev_data;

	/* platform data is a structure that we load with device info coming in from incoming argument */
	struct pcdev_platform_data *pdata;
	 ...
	 *
	 * where kzalloc is similar to kmalloc with only difference that it 0s out the entire memory  */
	...
	/* each device has its own private data structure, here we loading it through incoming platform_device struct's one
	 * of the field that we loaded in device file 
	 * */
...
/**
 * Functions that are needed to be called only once are called in init function.
 * Others get called in probe function.
 */
